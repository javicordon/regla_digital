function [ resolucion_S, maxima, minima, estandar, relacion_vs_rect ] = cos_rd( bits  )
...
t2=0:1:res_a-1;
plot(t,(res_a-1)/2*cos(t*2*pi()/periodo)+(res_a-1)/2);
...
for div=0:1:res_a-1
    %Con desfase por multiplexacion
    eqn = (res_a-1)/2*cos(x*2*pi()/periodo+2*pi()/2000)+(res_a-1)/2 == div;
    %En cuadratura perfecta
    %eqn = (res_a-1)/2*cos(x*2*pi()/periodo)+(res_a-1)/2 == div;
